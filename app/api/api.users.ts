...
export async function createUserAPI(user: CreateUserDto) {
    ...
    const response = await fetch('https://localhost:49167/api/User/create-user', {
      method: 'POST',
      headers: {
...
export async function getUserByIdAPI(id: string): Promise<UserDto> {
    const response = await fetch(`https://localhost:49167/api/User/${id}`,
    { cache: 'no-store' });
  ...
  export async function loginAPI(loginRequest: LoginRequestDto): Promise<LoginResponseDto>  {
    ...
    const response = await fetch('https://localhost:49167/api/User/login', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
